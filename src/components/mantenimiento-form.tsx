...
export function MantenimientoForm({ tipo, existing }: { tipo: string; existing?: any }) {
  ...
  const [ext, setExt] = useState({
    tag: existing?.equipo_externo?.tag ?? "",
    modelo: existing?.equipo_externo?.modelo ?? "",
    serie: existing?.equipo_externo?.serie ?? "",
    marca: existing?.equipo_externo?.marca ?? "",
    capacidad: existing?.equipo_externo?.capacidad ?? "",
    ubicacion: existing?.equipo_externo?.ubicacion ?? "",
  });
  ...
  const [datos, setDatos] = useState<Record<string, any>>(existing?.datos ?? {});
  ...
  const reloadEv = async () => { if (editId) setEvidencias(await listEvidencias({ mantenimiento_id: editId })); };

  useEffect(() => { if (!loading && !user) nav({ to: "/auth" }); }, [user, loading, nav]);
  useEffect(() => { reloadEv(); }, [editId]);
  ...
  async function guardar(finalizar: boolean) {
    ...
    if (modoExterno) {
      if (externoId) {
        ...
        extData = ext;
        if (!editId && guardarExterno && ext.tag) {
    ...
    let mantId = editId;
    if (editId) {
      const { error } = await supabase.from("mantenimientos").update(payload).eq("id", editId);
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-4">
        <Link to={editId ? "/mantenimiento/$id" : "/mantenimiento"} params={editId ? { id: editId } : undefined as any}
      ...
      <div className="mb-5">
        <div className="flex items-center gap-2 mb-1">
          ...
          {editId && <span className="text-[10px] px-2 py-0.5 rounded-full bg-warn/15 text-warn uppercase tracking-wider font-semibold">editando</span>}
        </div>
                ...
                {sec.items.map(it => (
                  <div key={it.k} className="space-y-1.5">
                    ...
                    {editId ? (
                      <PhotoCapture
                        mode="immediate"
                        parent={{ mantenimiento_id: editId }}
                        scope="parametro"
                        paramKey={it.k}
                        existing={evidencias.filter(e => e.scope === "parametro" && e.param_key === it.k)}
                        onChange={reloadEv}
                        label="Foto"
                        compact
                      />
      ...
      <div className="sticky bottom-20 mt-4 flex gap-2">
        <Button variant="outline" className="flex-1" disabled={busy} onClick={() => guardar(false)}>
          <Save className="size-4" /> {editId ? "Guardar cambios" : "Borrador"}
        </Button>
